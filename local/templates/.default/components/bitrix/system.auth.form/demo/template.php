	...
	<ul>
		<li class="att popup-wrap"><a id="hd_singin_but_open" href="" class="btn-toggle"><?=GetMessage("AUTH_LOGIN_LINK_TEXT")?></a>
			<form class="frm-login popup-block" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
				...
				<div class="frm-row">
                    <noindex><a href="<?=$APPLICATION->GetCurPageParam("forgot_password=yes", array(
                        "login",
                        "logout",
                        "register",
                        "change_password"));?>" class="btn-forgot" rel="nofollow"><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></a></noindex>
				</div>
				<div class="frm-row">
					<div class="frm-chk">
        ...
        <li><noindex><a href="<?=$APPLICATION->GetCurPageParam("register=yes", array(
                "login",
                "logout",
                "forgot_password",
